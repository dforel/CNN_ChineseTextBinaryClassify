...
def load_data_and_labels(jisuanji_data_file, jiaotong_data_file):
    ...
    jisuanji_examples = list(open(jisuanji_data_file, encoding='gb18030',errors='ignore').readlines())
    jisuanji_examples = [s.strip() for s in jisuanji_examples]
    jiaotong_exampless = list(open(jiaotong_data_file, encoding='gb18030',errors='ignore').readlines())
...
def load_test_data(test_file):
    ...
    test_data = list(open(test_file, encoding='gb18030',errors='ignore').readlines())
    ...
    x_text = [sent for sent in x_text] 
    # 定义类别标签 ，格式为one-hot的形式: y=1--->[0,1]
    positive_labels = [[0, 1] for _ in test_data]
